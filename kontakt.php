...
<?php
if(isset($_GET['kw'])) {
    $to = 'kontakt@webra.com.pl';
    $subject = $_POST['temat'];
    $message = 'Treść wiadomości: '."\n\n".$_POST['tresc'];
    $headers = 'From: '.$_POST['imie'].' <'.$_POST['email'].'>'. "\r\n";

    $wiadomosc = mail($to, $subject, $message, $headers);

    if($wiadomosc)
    {
        echo '<div style="border: 2px solid #00a000; background: #f5f5f5; padding: 20px;">Wiadomość wysłana</div>';
    }
    else
    {
        echo '<div style="border: 2px solid #00a000; background: #f5f5f5; padding: 20px;">Wystąpił problem z wysłaniem wiadomości</div>';
    }
}
?>
